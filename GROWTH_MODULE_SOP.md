...
The banner shows **two independently-generated, labeled lines**. Neither replaces the other; both are computed on every render, from the same `metrics` object.

**"UI:" line** (`#growth-prog-insight-text`) — unchanged, four branches evaluated in order:
...
**"Report:" line** (`#growth-prog-insight-report-text`, added 2026-09-24) — a separate, additional message for comparing this app's own generated summary against a report's phrasing/values during report testing, not a UI-testing message and not a replacement for the "UI:" line above. Generated by `buildGrowthProgressionReportInsight(bins)`, exposed on `window` for testing, computed entirely from `metrics.bins` (so it automatically respects whatever filter — e.g. Hide Harvested Plots — is currently applied, exactly like the "UI:" line already does). Two-sentence structure, modeled on a reference report summary style:
1. **Peak-range sentence**: names which progression-% range has the highest `slowPlots.length` ("Slow growth is highest in the {range} range"), then which range(s) peak for `normalPlots.length`/`fastPlots.length`. If normal and fast share the same peak bin, phrased as a single combined clause ("...while normal and fast growth peak in the {range} range."); if they peak in different bins, two separate clauses ("...while normal growth peaks in the {rangeN} range and fast growth peaks in the {rangeF} range."). If no plots show slow growth at all, a dedicated branch is used instead ("No plots show slow growth across any progression range.") — there is no case where the sentence is silently blank.
2. **Trend sentence**: compares each bin's `allPlots.length` total, specifically the **first bin (0-20%) vs. the last bin (80-100%)**, to decide the headline direction — `"tend to decline"` if the last bin's total is smaller, `"tend to increase"` if larger, or `"remain relatively consistent across all progression ranges"` if exactly equal. The named extreme range is the bin holding the **true minimum** (declining) or **true maximum** (increasing) total across *all 5* bins — not necessarily bin 0 or bin 4 themselves, since the middle bins can hold the actual extreme even when the overall first-vs-last direction is clear.
