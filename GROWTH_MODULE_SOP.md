...
### F. Insight banner — "UI:" and "Report:" lines (`#growth-prog-insight-banner`)
The banner shows **two independently-generated, labeled lines**. Neither replaces the other.

**"UI:" line** (`#growth-prog-insight-text`) — unchanged, four branches evaluated in order, computed from the (possibly Hide-Harvested-filtered) `metrics` object:
...
**"Report:" line** (`#growth-prog-insight-report-text`, added 2026-09-24) — a separate, additional message for comparing this app's own generated summary against a report's phrasing/values during report testing, not a UI-testing message and not a replacement for the "UI:" line above. Generated by `buildGrowthProgressionReportInsight(bins)`, exposed on `window` for testing. Two-sentence structure, modeled on a reference report summary style:

1. **Peak-range sentence**: for each of slow/normal/fast growth (only categories with at least one plot are considered), finds the bin with the highest count, then **groups categories that share the same peak bin into ONE combined clause** rather than repeating that range once per category. **Bug fixed 2026-09-25**: prior to this, slow's peak was always described in its own clause separately from normal/fast's — so a dataset where all three genuinely peaked in the same bin (e.g. a heavily skewed distribution with most plots concentrated in one progression range) produced a redundant sentence naming that same range twice ("Slow growth is highest in the 80-100% range, while normal and fast growth peak in the 80-100% range") instead of one combined clause. Current behavior, by number of distinct peak bins among the active categories:
   - **1 group** (all active categories share one bin): a single clause — `"Slow growth is highest in the {range} range."` (slow alone), `"{Category} growth peaks in the {range} range."` (one non-slow category alone), or `"{Slow, normal, and/or fast, Oxford-comma-joined} growth peak in the {range} range."` (2 or 3 categories merged).
   - **2 groups**: joined with `", while "` — e.g. `"Slow growth is highest in the {A} range, while normal and fast growth peak in the {B} range."` (unchanged from before for this specific shape) or `"Slow and normal growth peak in the {A} range, while fast growth peaks in the {B} range."` (a 2-way merge case that didn't exist before this fix).
   - **3 groups** (all three categories peak in different bins): joined with commas and a trailing `", and "` before the last clause — e.g. `"Slow growth is highest in the {A} range, normal growth peaks in the {B} range, and fast growth peaks in the {C} range."`
   - If no plots show slow growth at all, a dedicated branch is used instead (unchanged): `"No plots show slow growth across any progression range."` — normal/fast peaks are not described in this branch. There is no case where the sentence is silently blank.
2. **Trend sentence**: compares each bin's `allPlots.length` total, specifically the **first bin (0-20%) vs. the last bin (80-100%)**, to decide the headline direction — `"tend to decline"` if the last bin's total is smaller, `"tend to increase"` if larger, or `"remain relatively consistent across all progression ranges"` if exactly equal. The named extreme range is the bin holding the **true minimum** (declining) or **true maximum** (increasing) total across *all 5* bins — not necessarily bin 0 or bin 4 themselves. **This sentence can legitimately name the same range as sentence 1's peak clause** (e.g. both correctly say "80 - 100%" when one bin dominates every metric) — that is not the bug described above; the bug was sentence 1 repeating the *same* range within *itself*.
- Ties are broken by first occurrence (lowest bin index) throughout.
- If every bin's `allPlots.length` sums to `0`, returns `"No plots currently under active growth analysis."` (matching the "UI:" line's own empty-state wording). If `bins` is missing/empty, returns `"-"` defensively rather than throwing.
- Not tested end-to-end against any real external report — this reproduces a given reference example's *phrasing style*, not a verified 1:1 match to whatever a specific report-generation system actually outputs. Treat mismatches against a real report as expected until that report's exact algorithm is confirmed and this function is updated to match it.
- **Harvested-plot inclusion (corrected 2026-09-25)**: `buildGrowthProgressionReportInsight()` itself has no opinion on filtering — it just describes whatever `bins` it's given. `renderGrowthProgressionChart()` deliberately calls `computeProgressionMetrics(fullResults, totalPlotsCount, fullResults, false)` — a **second, separate** call from the one that produces the chart's own `bins`/`metrics` — passing `fullResults` (unfiltered) as both the results and `plotsToBin` arguments and `hideHarvested` forced to `false`. This means the **"Report:" line always includes harvested plots**, regardless of the Hide Harvested Plots checkbox, unlike the chart itself, its bubble overlay, its custom legend, and the "UI:" line — all four of which DO respect that checkbox (Section 6). This was a deliberate, explicit requirement, not an oversight: report-testing needs the full dataset even while the UI is being tested with harvested plots hidden.
...
The native Chart.js legend is disabled (`legend: { display: false }`) and replaced by a custom two-row HTML legend block, `#growth-prog-custom-legend`, rendered directly in `aggregate_dashboard_backup.html` below the canvas (populated in `renderGrowthProgressionChart()`). It reuses the exact same 3 dataset colors as the bars (`#f6c445` Slow, `#5cae57` Normal, `#264653` Fast — Normal and Fast interchanged 2026-09-25, see below) — colors are **not** redefined for the legend, only referenced.
...
Stacked bar chart (`type: 'bar'`, `stack: 'growthProgressionStack'`), 3 datasets: `Slow Growth` (`#f6c445`), `Normal Growth` (`#5cae57`), `Fast Growth` (`#264653`), `barThickness: 45`. **Corrected 2026-09-25**: Normal and Fast Growth's colors were interchanged so Normal Growth reads as green (`#5cae57`) and Fast Growth as the dark navy/teal (`#264653`) — previously the reverse. Applied identically to the bar dataset colors here, the KPI bar's category icons (Section 3E), and both rows of the custom legend (Section 3H) — all four locations were updated together so the color-to-category mapping is consistent everywhere on this chart. Clicking a bar drills into `showGrowthProgressionPlots(bins[index].label, bins[index].allPlots)`, whose table includes a `Daily Interpretation` column (the only one of the four drill-down tables that does).
...
```
**Only the Growth Progression chart is filtered by this checkbox** (per the code's own inline comments, quoted verbatim above). The Stage window, Harvest Window Weekly, and Harvest Window Daily charts always receive the full, unfiltered dataset. **The Progression chart's own "Report:" insight line (Section 3F) is a further exception within the Progression chart itself**: even though the chart's bars, bubble overlay, custom legend, and "UI:" line all respect this checkbox, the "Report:" line deliberately always uses the unfiltered dataset (added 2026-09-25, explicit requirement) — so on a single card, checking this box can visibly change everything except that one line.
...
Three Growth-specific regression tests exist in `Aggregate-Data-Testing/health_script.test.js`:
...
3. **`buildGrowthProgressionReportInsight_report_testing_message`** (added 2026-09-24) — loads `aggregate_script_backup.js` into the same `vm` sandbox pattern and exercises `buildGrowthProgressionReportInsight()` (Section 3F) directly against hand-built `bins` fixtures: a declining-trend scenario where normal and fast growth peak in the same bin (combined clause), an increasing-trend scenario where they peak in different bins (two separate clauses), a no-slow-growth scenario, an all-zero/no-data scenario, and defensive `null`/`[]` input handling.
...
* **2026-09-25**: Fixed the "Report:" Insight Line Redundantly Repeating the Same Peak Range, and Made It Always Include Harvested Plots:
  1. **Bug report**: with real data where slow, normal, and fast growth all genuinely peaked in the same bin (80-100%, a heavily skewed dataset), the message read "Slow growth is highest in the 80-100% range, while normal and fast growth peak in the 80-100% range" — mathematically correct in which range it named, but redundant, since slow's peak was always described in its own clause separately from normal/fast's regardless of whether they actually shared a bin.
  2. Fixed in `buildGrowthProgressionReportInsight()` (Section 3F): categories are now grouped by their peak bin index before building sentence 1, so any 2-or-3-way overlap collapses into one combined clause (e.g. `"Slow, normal, and fast growth peak in the {range} range."` when all three share a bin), while categories with genuinely different peak bins still get separate clauses (now also correctly handling a 3-way split with proper comma/"and" joining, and a slow+normal-merged/fast-separate 2-way split that wasn't previously distinguishable from the normal+fast-merged case).
  3. **Second, related fix from the same report**: the "Report:" line was being computed from the same (possibly Hide-Harvested-filtered) `bins` as the chart itself, but this line must always include harvested plots regardless of that checkbox. `renderGrowthProgressionChart()` now makes a **second, separate** `computeProgressionMetrics()` call using `fullResults` (unfiltered) for both the results and `plotsToBin` arguments and `hideHarvested` forced to `false`, feeding that unfiltered `bins` into `buildGrowthProgressionReportInsight()` instead of the chart's own filtered `bins` (Section 6).
  4. Added two new assertions to `buildGrowthProgressionReportInsight_report_testing_message`: one reproducing the exact reported all-three-same-bin scenario (asserting the peak range is named exactly once in sentence 1, with sentence 2's separate, legitimate mention of it as the overall trend max unaffected), and one for the slow+normal-merged/fast-separate 2-way split (35/35 tests passing — 0 new named test cases, extended the existing test).
* **2026-09-25**: Interchanged Normal Growth and Fast Growth Colors on the Growth Progression Chart — Normal Growth Is Now Green:
  1. Per explicit request, swapped the colors assigned to `Normal Growth` and `Fast Growth` so Normal Growth reads as green (`#5cae57`) and Fast Growth as the dark navy/teal (`#264653`) — previously the reverse. `Slow Growth` (`#f6c445`, orange/yellow) was not changed.
  2. Updated all 4 locations that hardcode these colors, kept in sync: the Chart.js bar dataset `backgroundColor` values (Section 3I), the KPI bar's `Normal Growth Plots`/`Fast Growth Plots` category icons (Section 3E), and both rows (Number of Plots, Usable Area) of the custom legend (Section 3H).
  3. No regression test change: this is a pure color/cosmetic swap with no logic, threshold, or computed-value change — colors aren't asserted anywhere in the test suite (35/35 tests passing, 0 new).
* **2026-09-24**: Added a Second "Report:" Insight Line to the Growth Progression Chart, Alongside the Existing "UI:" Line (For Report Testing):
  1. The existing insight banner (`#growth-prog-insight-text`, the "X plots on track / Y plots slow" sentence) is unchanged and now explicitly labeled `"UI:"` — kept exactly as-is, still needed for UI testing, per explicit request not to remove it.
  2. Added a second, independent line labeled `"Report:"` (`#growth-prog-insight-report-text`), populated by a new function `buildGrowthProgressionReportInsight(bins)`, for comparing this app's own generated summary against a report's phrasing during report testing. Both lines render inside the same `#growth-prog-insight-banner` container, now a `flex-direction: column` stack of two labeled rows instead of a single line (Section 3F).
  3. The new function reproduces a given reference example's two-sentence analytical style (peak progression-range per growth category, then an overall increasing/declining/steady trend statement naming the true extreme bin) — computed from `metrics.bins`, the same per-bin data the chart itself renders, so it automatically respects active filters like Hide Harvested Plots. It was **not** verified against any real external report-generation system's exact output — see the caveat in Section 3F.
  4. Added a dedicated regression test, `buildGrowthProgressionReportInsight_report_testing_message`, covering the combined-peak-bin phrasing, the split-peak-bin phrasing, the no-slow-growth branch, the all-zero-data branch, and defensive `null`/`[]` handling (35/35 tests passing — 1 new named test case; `baselineCount` bumped to 35 for future tasks per the ratchet rule).
