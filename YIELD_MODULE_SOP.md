...
2. **Unit Conversion (corrected 2026-09-23 — previously documented as using the global active-display-unit selector)**:
   - All trend points, reference lines, and tooltip metrics are expressed in the **plot's own crop-configured yield/harvest unit** (`d.yieldMassUnit`/`d.yieldAreaUnit`/`d.harvestUnit`, from the Variety API — Section 3B), matching the Yield/Harvest Analysis card exactly, using dynamic master conversion factors. They do **not** use the global active-display-unit selector (`getDataYieldUnit()`/`getDataHarvestUnit()`, driven by `#data-yield-unit`/`#data-harvest-unit`) — that selector only drives the aggregate cards and the base table's `Max Attainable` column (Section 3G.4), a deliberately separate display context.
   - **Bug fixed 2026-09-23**: `extractPlotMultiModelData()` previously converted the Forecasted/TASUMI/Biomass trend and tooltip to the global selector's unit while the Standard/Re-estimated/Maximum Attainable reference lines were already raw in the plot's own unit (see item 8 below) — putting two sets of lines on the very same chart in two different, unrelated unit scales, and neither matching the plot card. Both are now consistently in the plot's own unit.
3. **Visibility Rule**:
...
8. **Maximum Attainable Reference Line — Unit Basis (corrected 2026-09-23)**:
   - The trend chart's `stdVal`/`reVal` reference lines (`stdYield`/`reYield`, `stdHarvest`/`reHarvest`) are sourced **raw**, directly from `d.y1`/`d.y2`/`d.h1`/`d.h2` — never converted to the chart's active display unit (`getDataYieldUnit()`/`getDataHarvestUnit()`). They stay in the variety's own yield unit (Section 3B) because that's the basis `y1`/`h1` are already expressed in.
   - `extractPlotMultiModelData()`'s Maximum Attainable line now sources `maxAttainableYield`/`maxAttainableHarvest` the same way, **raw**, from `d.maxAttainableYieldPlotUnit`/`d.maxAttainableHarvestPlotUnit` (the same Section 3G.5 fields the plot card already displays) — since Maximum Attainable always shares Expected Yield's exact unit basis, no separate conversion is applied, consistent with `stdVal`/`stdHarvest`.
   - **Bug fixed 2026-09-23**: prior code instead re-derived a universal Tonnes/Ha value (`maxAttainableYieldTonHa`, computed independently from `varietyExpectedYieldUnits`/`varietyReferenceAreaUnits`) and ran it through `convertYield()`/`convertHarvest()` to the chart's display unit — a conversion `stdVal`/`reVal` never receive. This produced a Maximum Attainable reference line in a different unit scale than every other line on the same chart, rendering it pinned near zero (invisible against the Standard/Forecasted lines) whenever the variety's configured mass/area units differed from the chart's active display unit. The `maxAttainableYieldTonHa`/`convertYield` path is still used, unchanged, by the base table's `Max Attainable` column (Section 3G.4), which is a separate display context intentionally normalized to the user's active yield unit.
   - **Heuristic fallback removed (2026-09-23)**: `extractPlotMultiModelData()` previously synthesized a fallback reference line (`stdYield × 1.85`, or `maxYieldVal × 1.2` if no Standard Yield) whenever the variety had no real `maxAttainableYield` configured. This has been **removed entirely** — Maximum Attainable is a non-mandatory variety config value and must never be calculated or estimated. `maxAttainableYield`/`maxAttainableHarvest` are now `null` when the variety has none configured, and `createTrendChart()`'s existing `opts.maxVal > 0` gate simply omits the reference line from the chart in that case (no line drawn, nothing shown) — it is only ever plotted when a real configured value is present.
...
4. **Unit Normalization & Display**:
   ...
   - **`maxAttainableYield` sourcing (rolled back 2026-09-24)**: `extractVarietyYieldDetails()` (`server.js`, and its test-backing duplicate in `health_script.js`) reads `locEntry.maxAttainableYield` — the **flat** field, directly on the location entry, alongside `expectedYield`/`expectedYieldUnits`/`refrenceAreaUnits`. It does **not** read a nested `locEntry.data.maxAttainableYield`.
     - **History**: on 2026-09-23 this was briefly changed to read `locEntry.data.maxAttainableYield` (nested one level deeper, falling back to the flat field), because some upstream payloads at the time carried the value in that nested location instead of the documented flat one. This turned out to be an **upstream API bug** that duplicated `maxAttainableYield` into an extra nested `data: { maxAttainableYield: N }` object alongside the correct flat field — not a genuine schema change. The user confirmed the nesting is being removed as the source data is corrected, so on 2026-09-24 the extraction was rolled back to flat-only. **Do not reintroduce the nested-path read** unless a real upstream schema change is confirmed independently of this now-fixed bug.
...
5. **Plot-Level Yield/Harvest Card Display (added 2026-09-22, row-hiding removed 2026-09-23)**:
   - The plot's **Yield Analysis** card always shows a **Maximum Attainable Yield** row and the **Harvest Analysis** card always shows a **Maximum Attainable Harvest** row (`#plot-max-attainable-yield-row` / `#plot-max-attainable-harvest-row`), each labeled "(from variety config)". **Rows are never hidden (2026-09-23 change — previously hidden via the `hidden` class when absent)**: when the variety has no configured `maxAttainableYield`, the row displays literal `NA` text instead.
   - **Maximum Attainable Yield**: the raw `maxAttainableYield` from the Variety API is used directly (`maxAttainableYieldPlotUnit`), since it already shares the same `expectedYieldUnits`/`referenceAreaUnits` basis as `expectedYield` (Section 3B) — no conversion needed, consistent with how Expected Yield is now sourced. It is a non-mandatory config value and is never calculated or estimated when absent.
   ...
   - This is separate from `maxAttainableYieldTonHa` (Section 3G.4 above), which remains the universal Tonnes/Ha basis used only by the base table (Section 3D.8 covers the trend chart, which as of 2026-09-23 uses `maxAttainableYieldPlotUnit`/`maxAttainableHarvestPlotUnit` directly, not `maxAttainableYieldTonHa`).

---

## 4. Change Log (Feature & Logic Audit Trail)
* **2026-09-24**: Rolled Back `maxAttainableYield` Sourcing to Flat-Only — the 2026-09-23 Nested-Path Fix Was Working Around an Upstream API Bug That Is Now Being Corrected:
  1. On 2026-09-23, `extractVarietyYieldDetails()` was changed to read a nested `locEntry.data.maxAttainableYield` (falling back to the flat field) after observing that some upstream payloads carried the value there. The user has since confirmed this nested location was itself an **upstream API bug** — a temporary duplication of `maxAttainableYield` into an extra nested object alongside the correct flat field — and the source data is being corrected to remove the nesting, so it will no longer appear once fixed.
  2. Reverted `server.js` and its test-backing duplicate `Aggregate-Data-Testing/health_script.js` to read only the flat `locEntry.maxAttainableYield`, exactly as it worked before 2026-09-23. Do not reintroduce the nested-path read based on this incident — if a genuine schema change is ever confirmed independently, that would need fresh verification, not resurrecting this rolled-back fallback.
  3. Updated the `extractVarietyYieldDetails_and_maxAttainableYield_conversion` regression test: reverted its main fixture to the flat shape, and added a new assertion proving a nested-only payload (no flat sibling) now correctly resolves to `'NA'` — locking in that the nested path is no longer consulted, so this specific regression can't silently return without a test failure (34/34 tests passing — 0 new named test cases, extended the existing one).
* **2026-09-23**: Fixed Trend Chart Sourcing the Forecasted/TASUMI/Biomass Trend From the Wrong Unit — Now Matches the Plot Card Exactly:
  1. Root cause: `extractPlotMultiModelData()` converted the Forecasted/TASUMI/Biomass trend and its tooltip (`Max Predicted`/`Min Predicted`/`Average`) using `getDataYieldUnit()`/`getDataHarvestUnit()` — the global active-display-unit selector (a user-facing dropdown, defaulting to `kgs_acre`) — while the Standard/Re-estimated/Maximum Attainable reference lines on the exact same chart are sourced raw from `d.y1`/`d.y2`/`d.h1`/`d.h2`/`maxAttainableYieldPlotUnit`, which are in the crop's own configured unit (Section 3B), matching the plot card. Whenever the variety's configured unit differed from the global selector's unit, the chart displayed two internally inconsistent sets of numbers that also didn't match the card above it (e.g. card `Predicted: 7.12 - 7.87`, chart tooltip `Max Predicted: 2042.86 Kgs/Acre` for the same data point).
  2. Fixed in `aggregate_script_backup.js` and its test-backing duplicate `Aggregate-Data-Testing/health_script.js`: `extractPlotMultiModelData()` now derives `yieldUnit`/`harvestUnit` from `d.yieldMassUnit`/`d.yieldAreaUnit`/`d.harvestUnit` (the same fields the plot card already uses) instead of the global selector, so every value on the trend chart — Forecasted, Standard, Re-estimated, Maximum Attainable, and the tooltip — is in the exact same unit as the card. Introduced a local `convertYieldToPlotUnit()` helper (mass/area factors computed as separate arguments) instead of reusing `convertYield()`'s `"mass_area"` string-split API, since a Variety-API-sourced unit string could in principle contain an underscore and corrupt that split; `convertHarvest()` (single-unit, no split) is still reused as-is.
  3. `health_script.js`'s copy retains its `yieldUnitOverride`/`harvestUnitOverride` test parameters as a deterministic-testing escape hatch (existing tests explicitly passing them are unaffected), but now defaults to the plot's own unit — matching live behavior — when they're omitted. Added a new assertion (in the `extractVarietyYieldDetails_and_maxAttainableYield_conversion` test) that calls `extractPlotMultiModelData()` with no override and verifies the TASUMI trend converts using the plot's `yieldMassUnit`/`yieldAreaUnit` and lands in the same order of magnitude as `stdYield` (34/34 tests passing).
  4. The base table's `Max Attainable` column and the multi-model `T:`/`B:` comparison cells (`getPlotPredictionModelComparison()`, Section 3F) are unaffected — they remain intentionally normalized to the global active-display-unit selector, a separate display context (aggregate/multi-plot table, not the single-plot card+chart pairing).
* **2026-09-23**: Removed Heuristic Fallback for Maximum Attainable Yield/Harvest — Never Calculated, Only Shown When Configured:
  1. Maximum Attainable Yield/Harvest is a non-mandatory value configured on the variety in the source system — it must never be calculated, estimated, or synthesized by the dashboard. Removed the `stdYield × 1.85` / `maxYieldVal × 1.2` (and harvest equivalents) fallback heuristic from `extractPlotMultiModelData()` entirely (`aggregate_script_backup.js` and `Aggregate-Data-Testing/health_script.js`) — `maxAttainableYield`/`maxAttainableHarvest` are now `null` when the variety has no configured value, and the existing `opts.maxVal > 0` gate in `createTrendChart()` simply omits the reference line from the trend chart in that case (nothing drawn, no synthetic line).
  2. Plot card display changed from hide-when-absent to always-show-with-`NA` (Section 3G.5): `#plot-max-attainable-yield-row` / `#plot-max-attainable-harvest-row` are no longer toggled with the `hidden` class based on availability — they're always visible, and `updateElement()` now writes the literal text `NA` into `#plot-max-attainable-yield` / `#plot-max-attainable-harvest` when the variety has no configured value, instead of hiding the row. Updated in both the `noPrediction` branch and the normal `updatePlotPredictedDisplay()` path in `aggregate_script_backup.js`.
  3. Added a new assertion to the `extractVarietyYieldDetails_and_maxAttainableYield_conversion` regression test verifying `extractPlotMultiModelData()` returns `null` (not a heuristic estimate) for both `maxAttainableYield` and `maxAttainableHarvest` when the variety has no configured value (34/34 tests passing).
* **2026-09-23**: Fixed Trend Chart's Maximum Attainable Reference Line Rendering in the Wrong Unit Scale:
  1. Root cause: `extractPlotMultiModelData()` computed the chart's Maximum Attainable Yield/Harvest reference line by re-deriving a universal Tonnes/Ha value (`maxAttainableYieldTonHa`) and converting it to the chart's active display unit via `convertYield()`/`convertHarvest()` — but the chart's other reference lines (`stdVal`/`reVal`, from `d.y1`/`d.y2`/`d.h1`/`d.h2`) are used raw, in the variety's own configured unit, with no such conversion. Since Maximum Attainable always shares Expected Yield's exact unit basis (confirmed against the real Variety API sample: `expectedYield: 6.264802` and `maxAttainableYield: 7`, both in the same `TON`/`HECTARE` basis), converting only one of the two lines to a different unit scale caused the Maximum Attainable line to render pinned near zero whenever the variety's configured units differed from the chart's display unit — invisible against the Standard/Forecasted lines despite the plot card correctly showing the real value.
  2. Fixed in `aggregate_script_backup.js` and its test-backing duplicate `Aggregate-Data-Testing/health_script.js`: `extractPlotMultiModelData()` now sources `maxAttainableYield`/`maxAttainableHarvest` raw from `d.maxAttainableYieldPlotUnit`/`d.maxAttainableHarvestPlotUnit` (the same already-correct fields the plot card uses — Section 3G.5), matching how `stdVal`/`stdHarvest` are sourced, instead of re-deriving and converting a separate Tonnes/Ha value.
  3. The base table's `Max Attainable` column (Section 3G.4) is unaffected — it's a different display context that intentionally normalizes to the user's active yield unit via `maxAttainableYieldTonHa`/`convertYield()`, which was left unchanged.
  4. Updated the `extractVarietyYieldDetails_and_maxAttainableYield_conversion` regression test fixture (`plotWithVariety`) to set `maxAttainableYieldPlotUnit`/`maxAttainableHarvestPlotUnit` (raw, same basis as `y1`/`h1`) instead of `maxAttainableYieldTonHa`, asserting the chart now returns the raw values unconverted (34/34 tests passing).
