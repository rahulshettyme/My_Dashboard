...
## Growth Module SOP Constraint
- BEFORE proposing or executing any code modifications to the Growth module (Growth Progression, Stage window, Harvest window Weekly/Daily charts, base growth table), you MUST read the Growth Module SOP document at [GROWTH_MODULE_SOP.md](file:///c:/Users/rahul.shetty/Documents/Important/AntiGravity/My%20dashboard/GROWTH_MODULE_SOP.md) to understand binning rules, the backend extraction layer, and known caveats/bugs.


## SOP-First Lookup & Backfill Rule
- Whenever you need to look up a formula, logic rule, threshold, default value, field name, or behavior for the Health, Yield, or Growth module (e.g. to answer a question, or to understand existing behavior before making a change), **check the relevant SOP first** ([HEALTH_MODULE_SOP.md](file:///c:/Users/rahul.shetty/Documents/Important/AntiGravity/My%20dashboard/HEALTH_MODULE_SOP.md) / [YIELD_MODULE_SOP.md](file:///c:/Users/rahul.shetty/Documents/Important/AntiGravity/My%20dashboard/YIELD_MODULE_SOP.md) / [GROWTH_MODULE_SOP.md](file:///c:/Users/rahul.shetty/Documents/Important/AntiGravity/My%20dashboard/GROWTH_MODULE_SOP.md)) rather than immediately searching or re-deriving it from the codebase. If the SOP already documents it, use that as the answer.
...
- Once code changes are completed, perform the following verification workflow:
  ...
  2. **Regression Testing**: You MUST execute the regression test suite by running the exact command `node run_tests.js` from the project root on every code change. Running the suite any other way (e.g. manually calling `runSuite()`, invoking `health_script.test.js` directly, or running it from a subdirectory) does NOT satisfy this rule, even if it produces the same pass count — the exact command must be run. Report the counts of existing vs. new test cases in your response.
     - **`baselineCount` ratchet (`Aggregate-Data-Testing/health_script.test.js`)**: `baselineCount` means "tests that existed before this task's changes", NOT a fixed historical constant. Before reporting the existing/new split, check `tests.length` against the current `baselineCount`. If this task added N new test cases, `baselineCount` stays as-is so the report shows N new. If this task added zero new test cases, `baselineCount` MUST already equal `tests.length` — if it doesn't (i.e. it's stale from a prior task), update it to `tests.length` before reporting, so "new" reads 0 instead of re-reporting a prior task's additions as new again. After any task that does add new tests, bump `baselineCount` to the new `tests.length` as part of that same task, so the next task starts from an accurate baseline. Never let `baselineCount` sit unchanged across multiple tasks that added tests — it must track "count at the start of the current task" every time.
